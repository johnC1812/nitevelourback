...
// Keep /functions/api/live.ts catalog filtering consistent with the IDs used to
// generate static profile pages.
function writeGeneratedCatalogTS() {
  let ids = [];
  try {
    const cat = readJSON("data/catalog.json");
    if (cat && Array.isArray(cat.ids)) ids = cat.ids;
    else if (Array.isArray(cat)) ids = cat;
  } catch {
    ids = [];
  }
  ...
  const header = `// AUTO-GENERATED by tools/build.mjs\n// DO NOT EDIT BY HAND\n`;
  const body =
    `export const CATALOG_IDS = ${JSON.stringify(outIds)} as const;\n` +
    `export const CATALOG_SET: ReadonlySet<string> = new Set(CATALOG_IDS);\n`;

  writeFile("functions/_generated/catalog.ts", header + body);
}
...
function baseCSS() {
  ...
  <style>
    ...
    @media(min-width:900px){.cols4{grid-template-columns:repeat(auto-fill,minmax(200px,1fr))}.cols3{grid-template-columns:repeat(3,1fr)}}
    ...
    /* Live cards / buttons */
    .thumb{ display:block; width:100%; aspect-ratio:1/1; border-radius:12px; background:#0b0b0d; overflow:hidden; }
...
.gCard .meta{ flex:1; }
.gCard .actions{ margin-top:auto; display:flex; gap:10px; }
.gCard .actions a{ flex:1; text-align:center; }
...
function liveGridJS({
  ...
  const size = Number(pageSize) || 24;

  // IMPORTANT: avoid template literals in the client script itself because some HTML/JS minifiers can mangle them.
  return `
<script>
(function () {
  const state = {
    ...
    topic: ${JSON.stringify(topicDefault)},
    size: ${size},
  };
  ...
  const pills = Array.from(document.querySelectorAll(".pill[data-gender]"));
  ...
  // Mirror server-side safeId() so /live cards link to real /m/:brand/:id/ pages.
  function safeId(s) {
    return String(s || "")
      .toLowerCase()
      .replace(/[^a-z0-9_-]+/g, "")
  ...
  function makeCard(p) {
    const brand = safeText(p.systemSource || p.brand || p.site || "cam");
    const rawId = p.itemId || p.id || p.slug || (brand + "-" + safeText(p.nameClean || p.name || "model"));
    const id = safeId(rawId);
    const prof = "/m/" + encodeURIComponent(brand) + "/" + encodeURIComponent(id) + "/";

    const name = safeText(p.nameClean || p.name || id);
    const thumbUrl = safeText(p.thumbnailUrl || p.liveSnapshotURL || p.thumbUrl || p.thumbnail || p.avatarUrl || "");
    const watch = safeText(p.roomUrl || p.url || p.iframeFeedURL || prof);

    const card = document.createElement("div");
    card.className = "card2 gCard tile";
...
if (thumbUrl) {
      ...
      ph.style.cssText = "width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:#888;font-size:12px;";
    ...
    const g = p && p.characteristic && p.characteristic.gender ? safeText(p.characteristic.gender).toUpperCase() : "";
    const c = p && p.characteristic && p.characteristic.country ? safeText(p.characteristic.country) : "";
    if (g || c) {
      const mini = document.createElement("div");
      mini.className = "mini";
      mini.textContent = [g, c].filter(Boolean).join(" • ");
    ...
    liveBadge.className = "badge" + (p && p.live ? " live" : "");
    liveBadge.textContent = p && p.live ? "LIVE" : "OFFLINE";
    mini2.appendChild(liveBadge);

    const stars = Number((p && p.stars) || 0);
    if (stars > 0) {
      mini2.appendChild(document.createTextNode(" ") );
      const sb = document.createElement("span");
      sb.className = "badge";
      sb.textContent = "★ " + Math.min(5, Math.max(1, stars));
      mini2.appendChild(sb);
    }
    meta.appendChild(mini2);

    const actions = document.createElement("div");
    actions.className = "actions";

    const btnProfile = document.createElement("a");
    btnProfile.className = "btn secondary";
    btnProfile.href = prof;
    ...
    if (watch) {
      ...
      btnWatch.rel = "nofollow sponsored noopener";
      btnWatch.textContent = "Watch Live";
    } else {
      btnWatch.className = "btn secondary is-disabled";
      btnWatch.href = prof;
      btnWatch.setAttribute("aria-disabled", "true");
      btnWatch.tabIndex = -1;
      btnWatch.textContent = "Offline";
    }
  ...
  function applyActivePills() {
    pills.forEach((p) => {
      const g = p.getAttribute("data-gender") || "";
      if (g === state.gender) p.classList.add("pillActive");
  ...
  async function load(reset) {
    try {
      ...
      const url = "/api/live?" + qs(state);
      const res = await fetch(url, { headers: { accept: "application/json" } });
      const data = await res.json().catch(function () { return {}; });

      log("API status:", res.status, res.headers && res.headers.get ? res.headers.get("content-type") : "");
      log("keys:", data && typeof data === "object" ? Object.keys(data) : "(not object)");

      const items = data.items || data.models || data.performers || [];
      if (!Array.isArray(items) || items.length === 0) {
        if (state.topic && data.topicRequested && !data.topicApplied) {
          setStatus('No matches for "' + state.topic + '" — showing all live.');
        } else {
          setStatus("No live performers right now.");
        }
      ...
      setStatus("Error loading live performers. Check console.");
    }
  }

  pills.forEach((p) => {
    p.addEventListener("click", function () {
      state.gender = p.getAttribute("data-gender") || "";
      load(true);
    });
  });
...
(function(){
  ...
  const escHtml = (s) => String(s||"").replace(/[&<>"]/g, (c) => (c === "&" ? "&amp;" : (c === "<" ? "&lt;" : (c === ">" ? "&gt;" : "&quot;"))));
  const isLive = (v) => {
  ...
  function renderAlt(list){
    ...
    list.slice(0, 8).forEach((p) => {
      try{
        ...
        const id = safeId(p.nameClean || p.name || p.itemId || "");
  ...
  async function loadAlternatives(){
    ...
    u.searchParams.set("brands", "stripchat,chaturbate,awempire,streamate");
  ...
  async function findSelfLive(){
    try {
      const u = new URL("/api/live", location.origin);
      u.searchParams.set("page", "1");
      u.searchParams.set("size", "120");
      u.searchParams.set("brands", brand);

      const key = String(name || "").toLowerCase();
      for (const topicTry of [null, "all"]) {
        if (topicTry) u.searchParams.set("topic", topicTry);
        else u.searchParams.delete("topic");

        const d = await fetchJSON(u.toString());
        const list = (d && (d.performers || d.items || d.models)) || [];
        const hit = list.find((x) => {
          const n1 = String(x?.nameClean || x?.slug || x?.name || "").toLowerCase();
          const n2 = String(x?.itemId || "").toLowerCase();
  ...
  async function refresh(){
    ...
    if (p) {
      live = isLive(p.live);
      room = String(p.roomUrl || p.iframeFeedURL || "");
      iframeSrc = String(p.iframeFeedURL || "");
    }

    if (!live) {
      const hit = await findSelfLive();
      if (hit) {
        live = true;
        room = String(hit.roomUrl || hit.iframeFeedURL || room || "");
        iframeSrc = String(hit.iframeFeedURL || iframeSrc || "");
      }
    }
    ...
    if (live) {
      ...
      if (watchBtn) {
        ...
        watchBtn.tabIndex = 0;
        if (room) watchBtn.setAttribute("href", room);
      }
      if (iframe && iframeSrc) iframe.setAttribute("src", iframeSrc);
      if (iframeLink && room) iframeLink.setAttribute("href", room);
...
function buildModelPages(cfg, buildStamp) {
  ...
  for (let i=0; i<limit; i++) {
    ...
    const id = safeId(p.nameClean || p.name || p.itemId || `${bKey}-${slugify(p.nameClean || p.name)}`);
      ...
      <section class="card" style="margin-top:14px">
        ...
        <div class="grid cols4" style="margin-top:10px">
          ${related.map(r=>{
            ...
            const rid = safeId(r.nameClean || r.name || r.itemId || "");
...
function main() {
  ...
  // Refresh function-side catalog filter from the current sync output.
  writeGeneratedCatalogTS();
